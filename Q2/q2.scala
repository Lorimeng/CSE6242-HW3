...
val withoutDup = df.dropDuplicates()

// COMMAND ----------

// e.g. filter nodes by edge weight >= supplied threshold in assignment instructions
val filterData = withoutDup.filter($"Weight" >= 5).sort($"Source".asc)
//filterData.show()
...
val indegree = filterData.select("Target", "Weight").groupBy("Target").agg(sum("Weight")).sort(desc("sum(Weight)"), asc("Target"))
val highIndegree = indegree.limit(1).select("Target","sum(Weight)").withColumn("c", lit("i")).toDF("v", "d", "c")
//highIndegree.show()
...
val outdegree = filterData.select("Source", "Weight").groupBy("Source").agg(sum("Weight")).sort(desc("sum(Weight)"), asc("Source"))
val highOutdegree = outdegree.limit(1).select("Source","sum(Weight)").withColumn("c", lit("o")).toDF("v", "d", "c")
//highOutdegree.show()
...
val totaldegree = outdegree.union(indegree).groupBy("Source").agg(sum("sum(Weight)")).sort(desc("sum(sum(Weight))"), asc("Source"))
val highTotdegree = totaldegree.limit(1).select("Source","sum(sum(Weight))").withColumn("c", lit("t")).toDF("v", "d", "c")
//highTotdegree.show()
...
*/
val res = highIndegree.union(highOutdegree).union(highTotdegree)
//res.show()
